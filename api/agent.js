...
export default async function handler(req, res) {
  ...
  // --- TTS LOGIC (Microsoft SpeechT5) ---
  if (textToSpeak) {
    try {
      const ttsResp = await fetch("https://api-inference.huggingface.co/models/microsoft/speecht5_tts", {
  ...
  try {
    ...
    if (mode === 'image') {
      const imgResp = await fetch("https://api-inference.huggingface.co/models/black-forest-labs/FLUX.1-schnell", {
        headers: { Authorization: `Bearer ${keys.hf}`, "Content-Type": "application/json" },
        method: "POST",
        body: JSON.stringify({ inputs: prompt }),
      });
      if (!imgResp.ok) throw new Error("Image API Busy. Try again.");
      const buffer = await imgResp.arrayBuffer();
      const base64 = Buffer.from(buffer).toString('base64');
      return res.status(200).json({ data: `data:image/webp;base64,${base64}`, type: 'image' });
    }

    // --- VIDEO GENERATION (Wan 2.2) ---
    if (mode === 'video') {
      const vidResp = await fetch("https://api-inference.huggingface.co/models/Wan-AI/Wan2.1-T2V-14B", {
        headers: { Authorization: `Bearer ${keys.hf}`, "Content-Type": "application/json" },
        method: "POST",
        body: JSON.stringify({ inputs: prompt }),
      });
    ...
    if (mode === 'code') {
      ...
      const gemResp = await fetch(chatUrl, {
        method: 'POST',
