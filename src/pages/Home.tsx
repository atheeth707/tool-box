...
import { Search, TrendingUp, Grid, Sparkles } from 'lucide-react';
...
export default function Home() {
  ...
  return (
    <div className="space-y-6 md:space-y-16">
      ...
      {/* SEO */}
      <title>
        Free Online Tools - Image PDF QR AI Tools
      </title>

      <meta
        name="description"
        content="Free online tools for image conversion, PDF tools, QR generators, AI tools, compressors, background removers and more."
      />
          ...
          {searchQuery.length > 1 && (
            <div className="absolute top-full left-2 right-2 md:left-4 md:right-4 mt-1 md:mt-2 bg-white dark:bg-gray-800 rounded-lg md:rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 overflow-hidden z-50 text-left">
              ...
              {searchResults.length > 0 ? (
                <ul className="max-h-60 md:max-h-96 overflow-y-auto">
                  ...
                  {searchResults.map((tool: any) => (
                    <li key={tool.id}>
                      ...
                      <Link
                        to={`/tool/${tool.id}`}
                        className="flex items-center px-3 py-2 md:px-6 md:py-4 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors border-b border-gray-100 dark:border-gray-700 last:border-0"
                      >

                        <div className="bg-blue-100 dark:bg-blue-900/30 p-1.5 md:p-2 rounded md:rounded-lg mr-2 md:mr-4">
                          <Icon
                            name={tool.icon}
                            className="w-3 h-3 md:w-5 md:h-5 text-blue-600 dark:text-blue-400"
                          />
                        </div>

                        <div>

                          <div className="font-medium text-[11px] md:text-base text-gray-900 dark:text-white">
                            {tool.name}
                          </div>
                ...
                <div className="p-3 md:p-6 text-center text-[11px] md:text-base text-gray-500">
                  No results.
                </div>
              )}
      ...
      {!searchQuery && (
        <section>
          ...
          <div className="flex items-center space-x-2 md:space-x-3 mb-3 md:mb-6">
            ...
            <h2 className="text-sm md:text-3xl font-bold dark:text-white">
              Trending Tools
            </h2>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 md:gap-6">
            ...
            {categories.map((cat: any) => (
              <Link
                key={cat.id}
                to={`/category/${cat.id}`}
                className="group bg-white dark:bg-gray-800 p-2 md:p-8 rounded-lg md:rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm hover:shadow-lg transition-all flex flex-col items-center text-center space-y-1.5 md:space-y-3"
              >
                ...
                <div className="bg-blue-50 dark:bg-blue-900/20 p-2 md:p-5 rounded-lg md:rounded-xl group-hover:bg-blue-600 transition-colors">
                  ...
                  <Icon
                    name={cat.icon}
                    className="w-5 h-5 md:w-10 md:h-10 text-blue-600 dark:text-blue-400 group-hover:text-white"
                  />

                </div>

                <h3 className="font-semibold text-[11px] md:text-lg dark:text-white line-clamp-1">
                  {cat.name}
                </h3>
...
function ToolCard({ tool }: { tool: any }) {
  return (
    <Link
      to={`/tool/${tool.id}`}
      className="group bg-white dark:bg-gray-800 p-2 md:p-6 rounded-lg md:rounded-xl border border-gray-100 dark:border-gray-700 flex flex-col md:flex-row items-center md:items-start text-center md:text-left space-y-1 md:space-y-0 md:space-x-4 hover:shadow-lg transition-all"
    >
      ...
      <div className="bg-gray-50 dark:bg-gray-700/50 p-1.5 md:p-4 rounded-md md:rounded-lg shrink-0">
        ...
        <Icon
          name={tool.icon}
          className="w-4 h-4 md:w-6 md:h-6 text-gray-600 dark:text-gray-300"
        />
      ...
      <div className="min-w-0 w-full">
        ...
        <h3 className="font-semibold text-[11px] md:text-base dark:text-white line-clamp-1">
          {tool.name}
        </h3>

        <p className="hidden md:block text-sm text-gray-500 mt-1 line-clamp-1">
          {tool.description}
        </p>
