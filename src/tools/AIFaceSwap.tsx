...
const AIFaceSwap: React.FC = () => {
  ...
  // Custom element cast to bypass TS2339 build errors
  const GradioApp = 'gradio-app' as any;

  useEffect(() => {
    // Inject Gradio script for the AI engine
    ...
    script.onload = () => {
      // Rotate messages to keep the user engaged during long cold starts
      timerRef.current = setTimeout(() => setLoadStatus("Loading Neural Layers..."), 4000);
      
      // Wait for the custom element to be registered before hiding loader
  ...
  return (
    <div className="p-4 max-w-6xl mx-auto font-sans dark:bg-slate-950 transition-colors min-h-screen">
      ...
      <div className="mb-6 text-center">
        <h2 className="text-3xl font-black bg-gradient-to-r from-orange-500 to-red-600 bg-clip-text text-transparent uppercase tracking-tight">
          AI Face Swap
        </h2>
      ...
      <div className="mt-8 text-center text-[10px] text-gray-400 font-bold uppercase tracking-widest">
        Free & Unlimited Access • Tool 150+
      </div>
    </div>
  );
};
