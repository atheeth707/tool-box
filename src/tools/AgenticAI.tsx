    ...
    <div className="flex h-screen bg-[#050505] text-zinc-100 font-sans">
      <div className="flex-grow flex flex-col items-center">
        ...
        <div className="w-full max-w-5xl p-5 flex justify-between items-center border-b border-white/5 bg-black/40 backdrop-blur-2xl">
          ...
          <button onClick={() => { setMessages([]); localStorage.removeItem('arena_v5_data'); }} className="p-2 hover:bg-white/5 rounded-full text-zinc-700 transition-colors">
            <Trash2 size={16} />
          </button>
        </div>
        ...
        <div className="w-full max-w-3xl flex-grow overflow-y-auto px-6 py-10 space-y-12" ref={scrollRef}>
          {messages.map((m, i) => (
            <div key={i} className="flex gap-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
              ...
              <div className="flex-grow space-y-2 pt-1">
                {m.type === 'image' ? (
                  <img src={m.content} className="rounded-2xl border border-white/10 w-full shadow-2xl transition-all hover:scale-[1.01]" alt="AI" />
                ) : m.type === 'video' ? (
          ...
          {loading && <div className="flex items-center gap-2 text-zinc-600 text-xs animate-pulse pl-16"><Loader2 size={14} className="animate-spin"/> AI is generating your {mode}...</div>}
        </div>
          ...
          <div className="bg-[#0f0f0f] border border-white/10 rounded-[28px] p-2 shadow-2xl">
            <div className="flex gap-1 mb-2 px-1">
              {[
                ...
                <button key={t.id} onClick={() => setMode(t.id as any)} className={`flex items-center gap-2 px-4 py-1.5 rounded-full text-[9px] font-bold uppercase tracking-widest transition-all ${mode === t.id ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/40' : 'text-zinc-600 hover:text-zinc-300'}`}>
                  {t.icon} {t.id}
                </button>
              ))}
            </div>
            <div className="flex items-end gap-2 px-3 pb-1">
              <textarea rows={1} value={prompt} onChange={e => setPrompt(e.target.value)} onKeyDown={e => e.key === 'Enter' && !e.shiftKey && (e.preventDefault(), handleSend())} placeholder={`Describe your ${mode}...`} className="flex-grow bg-transparent p-2 outline-none text-sm text-zinc-100 placeholder-zinc-700 resize-none" />
              <button onClick={handleSend} className="bg-white text-black p-3 rounded-full hover:bg-zinc-200 active:scale-95 transition-all"><Send size={18} /></button>
            </div>
          </div>
        </div>
      </div>
