          ...
          {messages.map((m, i) => (
            <div key={i} className="flex gap-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
              ...
              <div className="flex-grow space-y-2 pt-1">
                {m.type === 'image' ? (
                  <img src={m.content} className="rounded-2xl border border-white/10 w-full shadow-2xl" alt="AI Output" />
                ) : m.type === 'video' ? (
          ...
          <div className="bg-[#0f0f0f] border border-white/10 rounded-[28px] p-2 shadow-2xl">
            <div className="flex gap-1 mb-2 px-1">
              {[
                ...
                <button 
                  ...
                  className={`flex items-center gap-2 px-4 py-1.5 rounded-full text-[9px] font-bold uppercase tracking-widest transition-all ${mode === t.id ? 'bg-blue-600 text-white shadow-lg' : 'text-zinc-600 hover:text-zinc-300'}`}
                >
                  {t.icon} {t.id}
                </button>
              ))}
            ...
            <div className="flex items-end gap-2 px-3 pb-1">
              <textarea 
                ...
                className="flex-grow bg-transparent p-2 outline-none text-sm text-zinc-100 resize-none placeholder-zinc-800" 
              />
