      ...
      <div className="flex-grow flex flex-col items-center">
        ...
        <div className="w-full max-w-5xl p-5 flex justify-between items-center border-b border-white/5 bg-black/40 backdrop-blur-2xl">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold tracking-tight text-sm uppercase opacity-70">Unified Multimedia Engine</span>
          </div>
          <button onClick={() => { setMessages([]); localStorage.removeItem('arena_v5_data'); }} className="p-2 hover:bg-white/5 rounded-full text-zinc-600 transition-all hover:text-white">
            <Trash2 size={16} />
          </button>
        </div>
        ...
        <div className="w-full max-w-3xl flex-grow overflow-y-auto px-6 py-10 space-y-12" ref={scrollRef}>
          {messages.map((m, i) => (
            <div key={i} className="flex gap-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
              ...
              <div className="flex-grow space-y-2 pt-1">
                {m.type === 'image' ? (
                  <img src={m.content} className="rounded-2xl border border-white/10 w-full shadow-2xl" alt="Generated" />
                ) : m.type === 'video' ? (
          ...
          {loading && (
            <div className="flex items-center gap-2 text-zinc-600 text-xs animate-pulse pl-16">
              ...
              AI is building your {mode}. This may take 15-30 seconds...
            </div>
          )}
        </div>
          ...
          <div className="bg-[#0f0f0f] border border-white/10 rounded-[28px] p-2 shadow-2xl">
            <div className="flex gap-1 mb-2 px-1">
              {[
                ...
                <button 
                  ...
                  className={`flex items-center gap-2 px-4 py-1.5 rounded-full text-[9px] font-bold uppercase tracking-widest transition-all ${mode === t.id ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/20' : 'text-zinc-600 hover:text-zinc-300'}`}
                >
                  {t.icon} {t.id}
                </button>
              ))}
            ...
            <div className="flex items-end gap-2 px-3 pb-1">
              <textarea 
                ...
                placeholder={`Type here for ${mode}...`} 
                className="flex-grow bg-transparent p-2 outline-none text-sm text-zinc-100 resize-none placeholder-zinc-700" 
              />
              <button onClick={handleSend} className="bg-white text-black p-3 rounded-full hover:bg-zinc-200 transition-all active:scale-90">
                <Send size={18} />
              </button>
            </div>
          </div>
